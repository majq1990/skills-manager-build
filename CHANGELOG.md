...
### Fixed
- **Enterprise login now survives a restart**: the token is saved encrypted at rest (settings table + AES-256-GCM, same protection as proxy credentials) and restored on startup; JWT expiry is checked locally, so re-login is only demanded when the token has actually expired instead of after a burst of 401s. Logging out clears the stored credentials too.
