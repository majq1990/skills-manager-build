...
### Fixed
- **Enterprise login now survives a restart**: the token is saved encrypted at rest (settings table + AES-256-GCM, same protection as proxy credentials) and restored on startup; JWT expiry is checked locally, so re-login is only demanded when the token has actually expired instead of after a burst of 401s. Logging out clears the stored credentials and the in-memory token (an empty token previously stayed set, leaving `is_authenticated` true).
- **Occasional crash on quit** (`cannot move state from Destroyed`): tray/Settings quit is now idempotent, so a second trigger can no longer call `exit()` on an already-dismantling event loop; the manual `window.destroy()` before exit is gone — `app.exit()` tears the window down itself, and that destroy call was posting window events into the winding-down loop, which is exactly what panicked.
- **Crash on Windows shutdown / logoff / restart** (same `cannot move state from Destroyed` panic; both the 2026-09-18 and 2026-09-24 log captures happened at shutdown time): tao 0.34.5's window subclass proc parks the event-loop runner state in `Destroyed` as soon as it sees `WM_ENDSESSION`, and the next dispatched event panics. A session-end guard now installs its own subclass proc on the main window: it intercepts `WM_ENDSESSION` first (swallowing it so tao never runs its handler) and posts `WM_QUIT`, letting the message loop exit through its normal path and shut down cleanly.
...
### Fixed
- **A clear explanation when the WebView2 install fails**: Windows installers keep the online runtime install (installer size unchanged), but when a machine carries a dead WebView2 registration (the bootstrapper reports "already installed for the system" and skips, leaving the app to die at startup with "Could not find the WebView2 Runtime"), the installer now verifies after copying files that a usable runtime really exists. When it does not, the user gets an explanation plus a one-click jump to the official download page instead of a silent install and an unexplained startup error.
- Reverted the 1.25.4 embedded-offline-runtime approach (274MB installers were not worth it; that build was never published to users).

## [1.25.4] - 2026-09-10

### Fixed
- Internal test build, never published to users (the embedded-offline-WebView2 attempt, superseded by 1.25.5's online-install + detection approach).
...
### Added
- Unified personal-memory repository shared by Claude Code, Codex, OpenCode, WorkBuddy, Cursor and other installed agents.
- Native memory discovery, SHA-256 deduplication, newest-mtime conflict resolution, automatic backups and audit logs.
- Universal memory bridge, startup sync, 60-second background reconciliation, GUI management page, and complete CLI workflow.

### Fixed
- Replaced the non-functional OpenCode pseudo-plugin with a universal bridge contract.
- Corrected the Windows memory symlink layout and added a safe copy fallback.

## [1.23.2] - 2026-07-09

### Fixed
- 反馈监控「关联技能」记录不准：此前把工具名（Read/Bash 等）当成技能名上报，现改为正确归属到真实企业技能。
- 修复反馈插件因缺失文件导致的 Claude Code / WorkBuddy 采集失效（hook 静默崩溃、零上报）。
- opencode 技能识别：opencode 以上下文注入方式加载技能（不走技能工具），改为从注入内容识别当前技能，报错才能正确归属。

### Added
- 反馈内容大幅增强：上报除描述外，新增「会话摘要」「卡点时间线（工具调用序列 + 报错）」「错误详情」，便于定位技能异常卡点。
- 反馈表新增分析列（会话摘要 / 卡点时间线 / 错误详情 / 详情ID）与全文取回接口，支撑后续问题分析。
...
### Added
- 恢复企业技能上传/发布功能（单个 + 批量）：在「我的技能」每个技能卡片新增「发布到企业」入口（UploadCloud 图标），打开发布弹窗可选填目标版本（留空由服务端自动递增），将本地技能打包上传到企业技能服务器并触发安全扫描；多选状态下工具栏新增「批量发布」按钮，逐个发布并展示每项的成功/失败状态与进度。安全扫描未通过或无上传权限时给出明确错误提示。

## [1.22.8] - 2026-06-15

### Added
- 企业技能市场：本地已安装的技能不再显示"安装"按钮，改为标记"已安装"（仍可点击重新拉取企业最新版覆盖更新）。
- 问题反馈/建议入口：在「设置 - 关于」工具栏新增「反馈 / 建议」按钮（工具反馈），并在「我的技能」每个技能卡片新增反馈入口（预填关联技能）。反馈经登录态提交到企业服务器，自动记录提交人与时间。
