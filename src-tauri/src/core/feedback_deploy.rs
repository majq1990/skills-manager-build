...
enum Method {
    ...
    CcSettings { settings_rel: &'static str, adapter: &'static str },
}
...
fn agents() -> Vec<AgentDeploy> {
    vec![
        AgentDeploy {
            ...
            method: Method::Opencode { plugins_rel: ".config/opencode/plugins" },
        },
        AgentDeploy {
            key: "claude_code",
            detect_rel: ".claude",
            method: Method::CcSettings { settings_rel: ".claude/settings.json", adapter: "claude-code" },
        },
        AgentDeploy {
            key: "workbuddy",
            detect_rel: ".workbuddy",
            method: Method::CcSettings { settings_rel: ".workbuddy/settings.json", adapter: "workbuddy" },
        },
    ]
}
...
fn cc_hooks(install_dir: &Path, adapter: &str) -> Value {
    let ingest = install_dir.join("adapters").join(adapter).join("ingest.mjs");
...
fn entry_command(entry: &Value) -> Option<String> {
    entry.get("hooks")?.as_array()?.first()?
        .get("command")?.as_str().map(|s| s.to_string())
}

/// 部署：把 bundle 拷到 install_dir，再对已检测到的 agent 落配置。返回已部署 agent key 列表。
pub fn deploy_from(bundle_src: &Path, home: &Path, install_dir: &Path) -> std::io::Result<Vec<String>> {
    ...
    for a in agents() {
        ...
        match a.method {
            Method::Opencode { plugins_rel } => {
                ...
                let adapter = install_dir.join("adapters").join("opencode").join("feedback-monitor.js");
                let loader = format!(
            ...
            Method::CcSettings { settings_rel, adapter } => {
...
mod tests {
    ...
    fn deploy_writes_loader_and_merges_hooks_idempotently() {
        ...
        touch(&bundle.join("adapters/opencode/feedback-monitor.js"), "export const FeedbackMonitor=async()=>({});");
        ...
        touch(&home.join(".claude/settings.json"),
        ...
        // opencode 加载器
        let loader = fs::read_to_string(home.join(".config/opencode/plugins/feedback-monitor.js")).unwrap();
        ...
        let s: Value = serde_json::from_str(&fs::read_to_string(home.join(".claude/settings.json")).unwrap()).unwrap();
        assert_eq!(s["model"], "x");
        ...
        let s2: Value = serde_json::from_str(&fs::read_to_string(home.join(".claude/settings.json")).unwrap()).unwrap();
