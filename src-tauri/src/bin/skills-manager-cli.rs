...
#[derive(Subcommand, Debug)]
enum MemoryCommand {
    /// Show LLM providers pulled from demo (with source: remote/cache/fallback).
    ListProviders {
        /// Bypass 24h cache and re-fetch from demo.
        #[arg(long)]
        refresh: bool,
    },
    /// Print environment health for the memory subsystem (URL, cache status, providers count).
    Doctor {
        /// Bypass cache when checking.
        #[arg(long)]
        refresh: bool,
    },
    /// Force refresh the local provider cache from demo.
    Refresh,
    /// Deploy shared memory (~/.agent-memory/*.md) as `memory-*` skills into every installed agent.
    Sync {
        /// Show what would be deployed without touching anything.
        #[arg(long)]
        dry_run: bool,
    },
    /// Show shared memory root + which installed agents would receive memories.
    Status,
    /// Import newer memories from native agent folders into the unified root.
    Migrate {
        /// Preview imports, replacements, backups, and deduplication only.
        #[arg(long)]
        dry_run: bool,
    },
    /// Create or update one canonical memory and distribute it to installed agents.
    Remember {
        #[arg(long)]
        title: String,
...
fn run(cli: Cli) -> anyhow::Result<()> {
    ...
    // Memory operations only need central_repo::cache_dir() (for the providers
    // cache) and network I/O — they don't touch the skills store, so run them
    // before the store lock is acquired. This lets `memory *` work even when
    // the GUI app or another CLI holds the skills repo lock.
    if let Commands::Memory(args) = cli.command {
        return run_memory(args, cli.json);
    }
...
fn run_adopt(
    ...
    // Build exclusion sets: existing skill names and central paths
    let mut excluded_names: std::collections::HashSet<String> = std::collections::HashSet::new();
    let mut excluded_paths: std::collections::HashSet<PathBuf> = std::collections::HashSet::new();
    for skill in store.get_all_skills()? {
        excluded_names.insert(skill.name.clone());
        let p = PathBuf::from(&skill.central_path);
        excluded_paths.insert(p.clone());
...
fn classify_adopt_candidate(
    ...
    if excluded_names.contains(&name) {
        skipped.push(AdoptCandidate {
            ...
            name,
            reason: "already managed (name exists in DB)".to_string(),
        });
        return;
    }

    if excluded_paths.contains(dir) || excluded_paths.contains(&canonical) {
        skipped.push(AdoptCandidate {
            path: dir.to_string_lossy().to_string(),
            name,
            reason: "already managed (path exists in DB or sync target)".to_string(),
        });
        return;
    }
...
struct MemoryProviderView {
    ...
    domestic: bool,
    requires_vpn: bool,
    recommended_rank: u32,
    canary_env: Option<String>,
    last_verified: Option<String>,
    status: String,
    notes: Option<String>,
}

impl From<&memory::Provider> for MemoryProviderView {
    fn from(p: &memory::Provider) -> Self {
        Self {
            id: p.id.clone(),
            ...
            canary_env: p.canary_env.clone(),
            last_verified: p.last_verified.clone(),
...
struct MemoryListReport {
    ...
    providers: Vec<MemoryProviderView>,
}

#[derive(Debug, Serialize)]
struct MemoryDoctorReport {
    ok: bool,
...
fn build_list_report(res: memory::LoadResult) -> MemoryListReport {
    let providers: Vec<MemoryProviderView> = memory::ranked(&res.registry)
        .into_iter()
        .map(MemoryProviderView::from)
        .collect();
    MemoryListReport {
        ok: true,
        source: res.source.as_str(),
        cache_path: res.cache_path.to_string_lossy().to_string(),
        cached_at: res.cached_at.map(format_system_time),
        fetch_error: res.fetch_error,
        schema_version: res.registry.schema_version,
        updated_at: res.registry.updated_at,
        provider_count: providers.len(),
        providers,
    }
}

fn build_doctor_report(res: memory::LoadResult) -> MemoryDoctorReport {
    let cache_path_str = res.cache_path.to_string_lossy().to_string();
    let cache_exists = res.cache_path.exists();
    let cache_fresh = matches!(res.source, memory::LoadSource::Cache) && res.fetch_error.is_none();
    MemoryDoctorReport {
        ok: true,
        providers_url: memory::provider_registry::providers_url(),
        ...
        cached_at: res.cached_at.map(format_system_time),
        fetch_error: res.fetch_error,
        provider_count: res.registry.providers.len(),
        schema_version: res.registry.schema_version,
        updated_at: res.registry.updated_at,
    }
}

fn run_memory(args: MemoryArgs, json: bool) -> anyhow::Result<()> {
    match args.command {
        MemoryCommand::ListProviders { refresh } => {
            let res = memory::load(refresh)?;
            print_json(&build_list_report(res), json);
        }
        MemoryCommand::Doctor { refresh } => {
            let res = memory::load(refresh)?;
            print_json(&build_doctor_report(res), json);
        }
        MemoryCommand::Refresh => {
            let res = memory::load(true)?;
            print_json(
                &serde_json::json!({
                    "ok": true,
                    "source": res.source.as_str(),
                    "cache_path": res.cache_path.to_string_lossy(),
                    "cached_at": res.cached_at.map(format_system_time),
                    "provider_count": res.registry.providers.len(),
                    "fetch_error": res.fetch_error,
                }),
        ...
        MemoryCommand::Migrate { dry_run } => {
            let report = memory::sources::reconcile(dry_run)?;
            print_json(&report, json);
        }
        MemoryCommand::Remember {
            ...
            print_json(
                &serde_json::json!({ "saved": saved, "sync": sync }),
                json,
            );
        }
    }
