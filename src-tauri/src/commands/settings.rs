...
pub async fn check_app_update(
    ...
    tauri::async_runtime::spawn_blocking(move || {
        ...
        // 自更新检查走自托管 demo 端点（与 tauri.conf.json plugins.updater.endpoints 同源），
        ...
        // demo latest.json 是 tauri updater 清单格式，版本在 "version" 字段（无 v 前缀）
        let tag = resp["version"]
            .as_str()
            .ok_or_else(|| AppError::network("No version in update manifest"))?;
        let latest_version = tag.strip_prefix('v').unwrap_or(tag).to_string();
        let release_url = UPDATE_DOWNLOAD_PAGE.to_string();
