...
// 当前会话正在跑的企业 skill（由 Skill 工具调用检测设置；session 重置即清）
export function setActiveSkill(sessionId, skill) {
  withLock(() => {
    const s = load();
    const cur = s.sessions[sessionId] || { tool: null, streak: 0, neg: 0, lastStuck: 0, errs: {} };
    cur.activeSkill = skill;
    s.sessions[sessionId] = cur; save(s);
  });
}
export function getActiveSkill(sessionId) {
  const s = load();
  return s.sessions[sessionId]?.activeSkill || null;
}

// 一轮结束：只清卡住计数，保留活动 skill 与错误防抖（skill 可能跨多轮）
export function resetCounters(sessionId) {
  withLock(() => {
    const s = load(); const c = s.sessions[sessionId];
    if (c) { c.streak = 0; c.neg = 0; c.lastStuck = 0; save(s); }
  });
}
