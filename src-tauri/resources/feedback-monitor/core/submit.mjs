...
function buildPayload({ type, skill, title, description, conversation, error, rounds, sessionId, toolTrail, failedTool }) {
  const isAuto = type === 'auto_stuck' || type === 'auto_error';
  // 会话上下文脱敏后存 sink（给分析用），放宽到 4000 字；钉钉表侧由服务端再截短展示
  const conv = conversation ? summarize(conversation, 4000) : null;
  const err = error ? sanitize(error) : null;
  // 工具调用时间线：逐行脱敏（分析卡点的核心材料），存 sink
  let trail, trailReview = false;
  if (Array.isArray(toolTrail) && toolTrail.length) {
    trail = toolTrail.slice(-30).map(s => { const r = sanitize(String(s)); if (r.needsReview) trailReview = true; return r.text; });
  }
  return {
    payload: {
      type, skill: skill || '',
      ...
      error: err ? { message: err.text, failedTool: failedTool || undefined } : undefined,
      toolTrail: trail,
    },
    needsReview: (conv?.needsReview || err?.needsReview || trailReview) === true,
    isAuto,
  };
}
