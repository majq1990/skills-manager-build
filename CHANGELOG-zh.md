...
### 修复
- **企业账号登录态不再每次重开都丢**：登录成功后 token 加密落库（`settings` 表 + AES-256-GCM，与代理凭据同级保护），启动时自动恢复；JWT 过期在本地判定，过期才要求重新登录，不必先撞一串 401。登出会同时清掉落盘凭据与内存态（此前传空串不会真正清除，`is_authenticated` 仍返回 true）。
- **退出时的偶发崩溃**（`cannot move state from Destroyed`）：托盘退出/设置退出加了幂等守卫，重复触发不会在事件循环拆除后再次 `exit()`；同时去掉退出前手动 `destroy()` 窗口——`app.exit()` 本身就会拆除窗口，那次 destroy 反而往正在收尾的事件循环里投递窗口事件触发 panic。
- **Windows 关机/注销/重启时的崩溃**（同样报 `cannot move state from Destroyed`，2026-09-18、09-24 两次日志均为关机时段）：tao 0.34.5 的窗口子类 proc 一收到 `WM_ENDSESSION` 就把事件循环状态永久停在 `Destroyed`，之后任何一次事件派发都会 panic。现在给主窗口加了一个会话结束守卫，在自己的子类 proc 里先拦下 `WM_ENDSESSION`（吞掉、不让 tao 看到）并 `PostQuitMessage(0)`，消息循环走正常退出路径干净关闭，不再 panic。
- **「skills repository is busy」报错**：中央仓库被其他操作占用时，安装 skill、管理 skill 到 Agent、git 备份等操作不再直接失败，改为等待最多 30 秒（锁释放后立即继续）。真正原因是后台托盘更新检查给每个 skill 单独加仓库锁，skill 数量多时锁几乎一直被占，把用户操作全部挡住——该检查只读 git remote / 本地路径并写数据库列，不碰中央仓库文件，已移除这把锁。
- **错误弹窗显示「[object Object]」**：Rust 侧 `AppError` 序列化后是 `{kind, message}` 对象，前端直接 `String(err)` 会显示成 `[object Object]`。现在所有 invoke 调用统一走一层包装，把结构化错误还原成可读文案，真实原因（如上面的锁冲突）不再被掩盖。

## [1.25.7] - 2026-09-16

### 发布概览
- 「报告问题」从「复制诊断信息自行粘贴到 GitHub issue」改为直接走内部反馈渠道，内部用户不再需要 GitHub 账号。

### 用户可见更新
- **「报告问题」直达反馈渠道**：设置页点击「报告问题」后自动收集诊断信息（版本、系统、最近日志摘要），直接打开反馈对话框并预填为描述，提交即进入内部反馈流程；不再弹「诊断信息已复制，请粘贴到 GitHub issue 中」——内部用户访问不了 GitHub，旧路径实际不可用。
- **反馈对话框支持预填与提交回调**：描述框打开时写入预填内容且用户可继续编辑；提交成功后自动清理 panic 横幅与 `last_panic.log` 记录。
- 日志摘要只保留最近 60 行（完整日志仍可走「导出日志」生成 zip），避免反馈描述过长。

### 开发者与治理更新
- 反馈对话框新增 `prefillDescription` / `onSubmitted` 两个 props；设置页移除对 clipboard-manager 插件的依赖。
...
### 修复
- **WebView2 安装失败时给出明确说明**：Windows 安装包维持在线安装（体积不变），但如果系统里存在失效的 WebView2 安装记录（引导器会误判“已为系统安装”而跳过，装完应用一启动就报 Could not find the WebView2 Runtime），安装完成后会立即检测运行时是否真正可用，缺失时弹出说明并提供一键跳转官方下载页手动安装，不再让用户装完面对一个没有解释的启动报错。
- 撤回 1.25.4 的内嵌离线运行时方案（安装包体积增至 274MB 的代价不划算，且该版本未对用户发布）。

## [1.25.4] - 2026-09-10

### 修复
- 测试期内部版本，未对用户发布（内嵌 WebView2 离线运行时的尝试，已被 1.25.5 的在线+检测提示方案取代）。
...
### 发布概览
- 修复统一记忆同步的非幂等写入：SKILL.md 生成头不再随每轮同步累积，历史膨胀文件自动自愈。

### 修复
- **记忆同步幂等化（P0）**：同步时检测源文件中残留的生成头（banner 注释与 `**Memory type**` 行）和生成式 frontmatter，剥离后再生成——无论生成产物被谁写回源目录，重复同步不会再让文件滚雪球（此前 273 个 memory skill 的 SKILL.md 每轮追加一段头部，单文件膨胀到约 2.7MB，合计约 1.7GB，把 WorkBuddy 的任务执行器内存压爆）。
- **历史膨胀自愈**：检测到带生成残留的源文件时，自动用净化后的规范形态原位回写，升级后首轮同步即完成存量清理，无需手工去重。
- **部署幂等**：目标 SKILL.md 内容未变化时跳过写入，不再每分钟无谓刷新 mtime 触发下游 agent 重载。
- **大小看门狗**：生成的 SKILL.md 超过 100KB 时记录告警日志，防止异常体积静默扩散到所有 agent。

### 验收
- 回归测试覆盖：连续 100 轮同步（含最坏情况"生成物被写回源目录"），目标文件字节数与 banner 计数恒定不变。
...
### 新增
- 本地 ZIP 导入与企业下载支持 multi-skill package，一个包可自动发现并独立安装多个子技能。
- 每个子技能记录包内相对路径 `source_subpath`，同时保留企业包名作为统一更新来源。

### 修复
- 企业自动更新按共享包来源解析子技能，同一个 multi-skill package 每轮只下载和处理一次。
- 企业技能包上传、下载超时提高到 5 分钟，支持体积较大的多技能归档。
- 企业下载失败时展示连接、TLS 或超时等底层原因。
- 有效父技能内部出现嵌套 `SKILL.md` 时保持整体安装，不错误拆分为多个技能。

## [1.24.0] - 2026-07-22

### 新增
- 跨 Agent 个人统一记忆仓，支持 Claude Code、Codex、OpenCode、WorkBuddy、Cursor 等已安装 Agent 共享调用记忆。
- 自动发现各 Agent 原生 memory 目录，支持 SHA-256 去重、按最新修改时间解决冲突、覆盖前备份和审计日志。
- 新增统一记忆写入桥、启动同步、每 60 秒后台对账、GUI 管理页面及完整 CLI 命令。

### 修复
- 移除无法真正接入 OpenCode 生命周期的伪插件，改为所有 Agent 通用的记忆桥协议。
- 修复 Windows 下错误的记忆符号链接目录结构，并在无符号链接权限时安全降级为复制。
...
### 修复
- 设置页版本号改为动态读取真实应用版本（此前写死为旧版本号）。
- 更新失败时的「下载页」跳转改指向 download.html（原目录地址会 403）。
...
### 用户可见更新
- 反馈监控：opencode / Claude Code / 腾讯 WorkBuddy 三个 agent 默认部署反馈插件，登录或安装企业技能时自动部署；支持连续卡住检测、报错上报、主动反馈三通道，上报前本地脱敏。
- 企业技能登录后自动更新到服务端最新版本。

### 开发者与治理更新
- 修正反馈接口路径（去掉多余的 `/api/v1`，修复手动反馈 404）。
